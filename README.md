...
本项目是**自建DNS、浏览器、小火箭**统一分流规则的一部分，通过严格同步dnsmasq-china-list，保证多端分流规则的一致性。
...
- **accelerated-domains.china.list**：主规则，包含10w多个适合直连的域名。据说以前有性能问题，但现在没什么感觉。
...
- **apple.china.list**：可选规则，推荐使用。Apple相关域名在国内有CDN加速，理论上适合直连。但如果你发现解析到了国外，就不要使用。
...
- **google.china.list**：可选规则，不推荐使用。你要是发现熟悉的网页上，一些图标突然变成了文字，就是直连gstatic.com不稳定导致的。
